...
class User
{
	...
	public function create(array $user) {
		$stmt = $this->conn->prepare('INSERT INTO users (name, email, mobile, department_id, designation_id, role, password) 
		VALUES (:name, :email, :mobile, :department_id, :designation_id, :role, :password)');
		$stmt->execute([
			'name' => trim($user['name']),
	...
	public function validate(array $user, bool $isEdit = false, ?int $excludeId = null): array
	{
		...
		if ($isEdit) {
			// If EITHER field is entered, both must be validated and match
	...
	public function paginate(int $page, int $perPage, string $search = '', string $sort = 'id', string $order = 'asc', ?int $departmentId = null, ?int $designationId = null): array {
		...
		// Whitelist of valid sorting columns to prevent SQL injection
		$sortColumns = [
			'id'          => 'u.id',
			'name'        => 'u.name',
			'email'       => 'u.email',
